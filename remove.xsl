...
>
    ...
    <xsl:key
        name="DotfilesToRemove"
        match="wix:Component[ substring( wix:File/@Source, string-length( wix:File/@Source ) - 6 ) = 'gitkeep' ]"
        use="@Id"
    />
	...
	<xsl:template match="*[ self::wix:Component or self::wix:ComponentRef ][ key( 'DotfilesToRemove', @Id ) ]" />
